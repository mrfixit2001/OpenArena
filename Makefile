#
# ioq3 Makefile (with Open Arena modifications)
#
# GNU Make required
#
...
ifneq ($(BUILD_GAME_SO),0)
  TARGETS += \
    $(B)/baseoa/cgame$(SHLIBNAME) \
    $(B)/baseoa/qagame$(SHLIBNAME) \
    $(B)/baseoa/ui$(SHLIBNAME)
...
$(B)/baseoa/cgame$(SHLIBNAME): $(Q3CGOBJ)
	$(echo_cmd) "LD $@"
...
$(B)/baseoa/qagame$(SHLIBNAME): $(Q3GOBJ)
	$(echo_cmd) "LD $@"
...
ifneq ($(BUILD_GAME_SO),0)
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/baseoa/cgame$(SHLIBNAME) \
					$(COPYDIR)/baseoa/.
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/baseoa/qagame$(SHLIBNAME) \
					$(COPYDIR)/baseoa/.
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/baseoa/ui$(SHLIBNAME) \
					$(COPYDIR)/baseoa/.
  ifneq ($(BUILD_MISSIONPACK),0)
	-$(MKDIR) -p -m 0755 $(COPYDIR)/missionpack
